...
bool GameOver::init()
{
	...
	if (CocosDenshion::SimpleAudioEngine::getInstance()->isBackgroundMusicPlaying())
	{
		...
		CocosDenshion::SimpleAudioEngine::getInstance()->playBackgroundMusic("sound/game_over.mp3");
	}
	...
	do 
	{
		...
		//添加背景图片
		auto m_background = Sprite::createWithTexture(TextureCache::getInstance()->getTextureForKey("backgroundGameOver.jpg")); //SpriteFrameName("backgroundGameOver.jpg");
		...
		//标签闪光动作
		auto m_tintBy = TintBy::create(1.0f, 255, -255, -255);
		...
		score->setColor(Color3B(0, 255, 255));
		score->runAction(RepeatForever::create(m_seq));
		CC_BREAK_IF(!score);
		...
		m_label1->runAction(Sequence::create(m_seq, DelayTime::create(1.0f), m_seq,nullptr));
...
void GameOver::menuMainCallback(cocos2d::Ref* pSender)
{
	...
	Director::getInstance()->replaceScene(TransitionProgressRadialCCW::create(2.0f,HelloWorld::createScene()));
}